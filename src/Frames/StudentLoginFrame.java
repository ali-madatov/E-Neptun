...
public class StudentLoginFrame implements ActionListener{
	...
	public StudentLoginFrame(){
		...
		bottomPanel.setBackground(Colors.backgroundColor);
	...
	private void createRegisterPanel() {
		...
		registerLabel = new JLabel("Are not you student yet?");
		registerLabel.setFont(new Font(Font.SERIF,Font.ITALIC,13));
		registerLabel.setPreferredSize(new Dimension(135,30));
		registerLabel.setForeground(Color.white);
		
		registerButton = new JButton();
		registerButton.setPreferredSize(new Dimension(70,20));
		registerButton.setFocusable(false);
		registerButton.setText("Apply for it");
		registerButton.setFont(new Font(Font.SERIF,Font.ITALIC,13));
	...
	private boolean loginAccount(String username,String password) {
		Optional<Map.Entry<Integer,Student>> matchingObject = Controller.students.entrySet().stream().
			    filter(p -> p.getValue().getLoginName().equals(username) && p.getValue().getPassword().equals(password)).
			    findAny();
		if(matchingObject.isPresent()) {
			new StudentAccount(matchingObject.get().getValue());
			frame.dispose();
		}
