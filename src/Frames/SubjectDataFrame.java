...
public class SubjectDataFrame {
	JFrame frame;
	JScrollPane scrollPane;
	
	public SubjectDataFrame(Subject subject) {
		frame = new JFrame();
		frame.setLayout(new BorderLayout());
		frame.setTitle("Subject Info");
		...
		if(subject.getClassIDs()!=null && subject.getClassIDs().size()>0) {
			mainPanel.setPreferredSize(new Dimension(1000,Math.max(subject.getClassIDs().size()*60,400)));
		}
		if(subject.getClassIDs()!=null)
		for(int i: subject.getClassIDs()) {
			JPanel dataPanel = new JPanel();
			dataPanel.setPreferredSize(new Dimension(960,40));
...
//			classroom.setSchedule(new Schedule(Schedule.WeekDay.MONDAY,LocalTime.now(),LocalTime.NOON));
	...
	public SubjectDataFrame(Subject subject,Student student) {
		JFrame frame;
		JScrollPane scrollPane;
		Set<ClassRoom.SubjectType> subjectTypes;
		
			subjectTypes = new HashSet<ClassRoom.SubjectType>();
			subjectTypes.add(ClassRoom.SubjectType.LAB);
			subjectTypes.add(ClassRoom.SubjectType.LECTURE);
			subjectTypes.add(ClassRoom.SubjectType.PRACTICE);
...
//				classroom.setSchedule(new Schedule(Schedule.WeekDay.MONDAY,LocalTime.now(),LocalTime.NOON));
				...
				input = timing;
				JLabel Label5 = new JLabel(input);
				Label5.setPreferredSize(new Dimension(180,40));
				Label5.setFont(new Font(Font.DIALOG,Font.BOLD | Font.ITALIC,14));
				Label5.setForeground(Color.green);
				
				JCheckBox moreButton = new JCheckBox();
				...
				if(student.getClassIDs()!=null && student.getClassIDs().contains(classroom.getClassRoomID()))
